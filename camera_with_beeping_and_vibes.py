...
# these frequency, length parameters are touchy... idk how they work @Kim
def beeper(stream, frequency=440, length=0.1, rate=44100):
    chunks = []
    chunks.append(sine(frequency, length, rate))
    chunk = np.concatenate(chunks) * 0.25
    stream.write(chunk.astype(np.float32).tostring())

# Uses clock time to control beeping and vibration
def beep_n_vibe(m, stream, param, last_time, frequency=440, length=0.1, rate=44100):
    # mapping function
    delay = SCALE * (param/NORM) ** (1/ORDER) + MIN_FT
    if time.time() > last_time + delay:
        m.vibrate("short")
        beeper(stream, frequency=frequency, length = length, rate = rate)
        vib_time = time.time()
        print(delay)
        return vib_time
    else:
